...
# pre-processing function (this might need to be
...
# Load our the network weights from disk (NOTE: if this is the first time you are
# running this script for a given
# network, the weights will need to be downloaded first -- depending on which
# network you are using, the weights can
# be 90-575MB, so be patient; the weights will be cached and subsequent runs
# of this script will be *much* faster)
...
# image is resized to `inputShape`, the required
...
# (inputShape[0], inputShape[1], 3) however we need to
# expand the dimension by making the shape (1, inputShape[0], inputShape[1], 3)
